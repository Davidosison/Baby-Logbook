...
const WHATS_NEW_VERSION = "2026-09-02-dashboard-redesign";
const STORAGE_KEY = "whats-new-seen";

const ITEMS_HE = [
  "🩹 ה\"קקי\" ו\"שניהם\" הוחלפו בכפתורי \"סימיקול\" (בחירת מ\"ל) ו\"ג'ל לחניכיים\" (מותג + חניכיים)",
  "📊 כרטיסי הבית מציגים עכשיו גם סה\"כ יומי — מ\"ל, שעות שינה ומספר חיתולים",
  "😴 זמן השינה האחרון מחושב מרגע ההתעוררות, לא מרגע ההירדמות",
  "🕒 ציר הזמן בדף הבית מעוצב מחדש עם צבעים לפי סוג וקו מחבר",
  "📅 בהיסטוריה: חיפוש חופשי, מיון לפי חודשים ושבועות, ואפשרות לכווץ קטגוריות",
];

const ITEMS_RU = [
  "🩹 Кнопки «Ка-ка» и «Оба» заменены на «Симикол» (выбор мл) и «Гель для дёсен» (бренд + дёсны)",
  "📊 На карточках теперь видны дневные итоги — мл, часы сна и число подгузников",
  "😴 Время последнего сна теперь считается от момента пробуждения",
  "🕒 Обновлён дизайн ленты событий на главном экране",
