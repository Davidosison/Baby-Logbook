...
export default function SleepPage() {
  ...
  const startSleep = useStartSleep({ mutation: { onSuccess: invalidateAll } });
  const stopSleep = useStopSleep({
    mutation: {
      onSuccess: () => { invalidateAll(); setLocation("/"); },
    },
  });
  ...
  return (
    <div className="min-h-[100dvh] bg-background pb-32 flex flex-col" dir={dir}>
      <PageHeader hebrewTitle="שינה" russianTitle="Сон" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 pb-20">
        {isSleeping ? (
          <div className="text-center mb-12">
            ...
            <div className="text-lg text-muted-foreground font-medium">{tr("asleep", lang)}</div>
          </div>
        ) : (
          <div className="text-center mb-12">
            <div className="text-2xl text-muted-foreground font-medium">{tr("babyAwake", lang)}</div>
          </div>
        )}
        ...
        >
          <div className="text-4xl font-bold mb-2">
            {isSleeping ? tr("wakeUp", lang) : tr("startSleepLabel", lang)}
          </div>
          <div className="text-lg opacity-80">
            {isSleeping ? tr("stopSleep", lang) : tr("startSleepSub", lang)}
          </div>
        </button>
      </div>
    </div>
