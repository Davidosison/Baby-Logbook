...
import { format, isToday, isYesterday, addDays, subDays, startOfDay } from "date-fns";
import { he, ru } from "date-fns/locale";
import { Droplet, Moon, Utensils, Trash2, Pencil, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function HistoryPage() {
  ...
  const eventsForDate = events?.filter(
    (e) => format(new Date(e.startedAt), "yyyy-MM-dd") === selectedDateStr
  ) ?? [];
    ...
    <div className="min-h-[100dvh] bg-background pb-32" dir={dir}>
      ...
      <div className="flex items-center justify-between px-4 py-3" dir="ltr">
        <button
          ...
          className="w-10 h-10 flex items-center justify-center rounded-full bg-card border border-border text-muted-foreground hover:text-foreground transition-colors active:scale-95"
        >
          <ChevronLeft className="w-5 h-5" />
        </button>
        <span className="font-semibold text-base">{dateHeading(selectedDate)}</span>
        <button
          onClick={() => setSelectedDate((d) => addDays(d, 1))}
          disabled={isFuture}
          className={cn(
            "w-10 h-10 flex items-center justify-center rounded-full bg-card border border-border transition-colors active:scale-95",
            isFuture ? "opacity-30 cursor-not-allowed" : "text-muted-foreground hover:text-foreground"
          )}
        >
      ...
      <div className="p-4 space-y-2">
        ...
        {!isLoading && eventsForDate.length === 0 && (
